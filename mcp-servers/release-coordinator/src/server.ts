...
import { getDirname, findProjectRoot, loadEnvManually } from '@mcp-servers/shared';
import path from 'path';
import fs from 'fs';
import { WeeklyBlockersService } from './weekly-blockers.service.js';

// Load environment variables using shared utility (manual loading for this server)
...
interface OverviewArgs {
  ...
  testManagerUpdate?: string; // Test manager's release decision message
...
export class ReleaseCoordinatorServer {
  ...
  constructor() {
    // Load MCP config to get tokens (use project root from shared utility)
    const mcpConfigPath = path.resolve(projectRoot, '.vscode/mcp.json');
  ...
  private setupHandlers(): void {
      ...
      tools: [
        {
          ...
          inputSchema: {
            ...
            properties: {
              ...
              testManagerUpdate: { type: 'string', description: 'Markdown or text from Slack get_test_manager_update (test manager decision)' },
        ...
        {
          name: 'get_weekly_blockers_report',
          description:
            'Compile all blocker tickets from the current week\'s test manager summary messages. Groups tickets by team with Jira details (labels, components, parent).',
          inputSchema: {
            type: 'object',
            additionalProperties: false,
            properties: {
              channel: { type: 'string', description: 'Slack channel to scan (defaults to functional-testing)' },
              weekOffset: { type: 'number', description: '0 = current week (default), -1 = previous week, -2 = two weeks ago, etc.', default: 0 },
              postToSlack: { type: 'boolean', description: 'If true, posts the report to #qa-release-status', default: false },
            },
            required: [],
          },
        },
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      switch (request.params.name) {
        ...
        case 'get_weekly_blockers_report':
          return this.handleGetWeeklyBlockersReport((request.params.arguments ?? {}) as WeeklyBlockersArgs);
        default:
  ...
  private async handleGetComprehensiveReleaseOverview(args: OverviewArgs) {
    ...
    const [jiraSummary, autoStatus, blockers, testManagerUpdate] = await this.fetchSectionsIfNeeded(args);
    ...
    // Build the response, only including test manager section if found
    let response = `🚀 *Release Status Overview — ${dateLabel}*
...
${blockers}`;
    ...
    // Add test manager decision at the end if found (not just placeholder text)
    if (testManagerUpdate && !testManagerUpdate.includes('No test manager update found')) {
      response += `\n${testManagerUpdate}`;
    }

    response += `\n---
_Generated by MCP Release Coordinator • Sections may include links from source servers_`;
  ...
  private async handleGetWeeklyBlockersReport(args: WeeklyBlockersArgs) {
    const channel = args.channel || 'functional-testing';
    const weekOffset = args.weekOffset ?? 0;
    const service = new WeeklyBlockersService(this.getDirname(), () => this.buildEnv());

    try {
      const report = await service.generateReport(channel, weekOffset);
      const formatted = service.formatReport(report);

      // Optionally post to Slack
      if (args.postToSlack) {
        try {
          const postResult = await this.callSlackSendMessage({ channel: 'C09BW9Y2HSN', text: formatted });
          console.error('Posted weekly blockers to Slack:', postResult);
          const postedNote = postResult ? '\n\nPosted to #qa-release-status.' : '';
          return { content: [{ type: 'text', text: formatted + postedNote }] };
        } catch (error) {
          console.error('Failed to post to Slack:', error);
      ...
      return { content: [{ type: 'text', text: `Error generating weekly blockers report: ${error?.message || String(error)}` }] };
    }
  }

  private async fetchSectionsIfNeeded(args: OverviewArgs): Promise<[string, string, string, string]> {
    const needsJira = !args.jiraTestingSummary;
    const needsAuto = !args.autoTestStatus;
    const needsBlockers = !args.blockingIssues;
    const needsTestManager = !args.testManagerUpdate;

    if (!needsJira && !needsAuto && !needsBlockers && !needsTestManager) {
      return [
        args.jiraTestingSummary!.trim(),
        args.autoTestStatus!.trim(),
        args.blockingIssues!.trim(),
        args.testManagerUpdate!.trim()
      ];
    }
    ...
    const order: Array<'jira' | 'auto' | 'blockers' | 'testManager'> = [];
    ...
    if (needsTestManager) {
      order.push('testManager');
      tasks.push(this.callSlackGetTestManagerUpdate({ channel, date }));
    }
    ...
    let testManagerOut = args.testManagerUpdate?.trim();
    for (let i = 0; i < order.length; i++) {
      ...
      else if (key === 'testManager') testManagerOut = val;
    }
  ...
  private async callSlackGetAutoTestStatus(params: { channel: string; date: string }): Promise<string> {
    ...
    try {
      ...
      } as any, undefined, { timeout: 90000 }); // 90s timeout (default is 60s)
  ...
  private async callSlackGetTestManagerUpdate(params: { channel: string; date: string }): Promise<string> {
    const client = new Client({ name: 'release-coordinator', version: '1.0.0' });
    const transport = new StdioClientTransport({
      command: 'node',
      args: ['dist/server.js'],
    ...
    try {
      ...
      // Don't show error for test manager update - it's optional
      return '';
    } finally {
      await client.close?.().catch(() => undefined);
    }
  ...
  private getDirname(): string {
    // Use the module-level __dirname from shared utility
    return __dirname;
  }
  ...
  private buildEnv(): Record<string, string> {
    ...
    const allEnvVars = { ...slackEnv, ...jiraEnv, ...confluenceEnv };
